...
describe('Header Component', () => {
  const renderHeaderWithAuth = (authProviderValue: Partial<AuthContextType>) => {
    const currentContextValue = { ...initialMockAuthContextValue, ...authProviderValue };
    return render(
  ...
  test('renders brand name "Smart Auto Trader"', () => {
    ...
    expect(screen.getByText((content, element) => content.startsWith('Smart Auto Trader') && element?.tagName.toLowerCase() === 'a')).toBeInTheDocument();
  });
  ...
  describe('when user is not authenticated and not loading', () => {
    test('renders "Log In" and "Register" links and does not show profile or logout', () => {
      ...
      expect(screen.queryByRole('link', { name: 'Admin Inquiries' })).not.toBeInTheDocument();
    });
  });
  ...
  describe('when user is authenticated and not loading', () => {
    ...
    describe('Admin Links', () => {
      ...
      test('renders "Admin Inquiries" link for authenticated admin user', () => {
        ...
        expect(screen.getByRole('link', { name: 'Admin Inquiries' })).toBeInTheDocument();
        expect(screen.getByRole('link', { name: 'Admin Inquiries' })).toHaveAttribute('href', '/admin/inquiries');
      });
    });
  });

  describe('when auth state is loading', () => {
    const mockUserWhileLoading: User = { id: 3, username: 'LoadingUser', email: 'loading@example.com', firstName: 'Still', lastName: 'There', roles: ['User']};
    const mockAdminUserWhileLoading: User = { id: 4, username: 'LoadingAdmin', email: 'loadingadmin@example.com', firstName: 'AdminStill', lastName: 'There', roles: ['Admin', 'User']};
    ...
    test('renders "Log In" and "Register" links if not authenticated, even when loading', () => {
      renderHeaderWithAuth({ loading: true, isAuthenticated: false, user: null });
      expect(screen.getByRole('link', { name: 'Log In' })).toBeInTheDocument();
      expect(screen.getByRole('link', { name: 'Register' })).toBeInTheDocument();
      expect(screen.queryByRole('link', { name: 'My Profile' })).not.toBeInTheDocument();
      expect(screen.queryByRole('button', { name: 'Log Out' })).not.toBeInTheDocument();
    ...
    test('renders Profile, Logout, and Admin (if applicable) links if authenticated, even when loading', () => {
      renderHeaderWithAuth({ loading: true, isAuthenticated: true, user: mockAdminUserWhileLoading });
      expect(screen.getByRole('link', { name: 'My Profile' })).toBeInTheDocument();
      expect(screen.getByRole('button', { name: 'Log Out' })).toBeInTheDocument();
      expect(screen.getByRole('link', { name: 'Admin Inquiries' })).toBeInTheDocument();
      expect(screen.queryByRole('link', { name: 'Log In' })).not.toBeInTheDocument();
    ...
    test('renders Profile and Logout links but NOT Admin link if authenticated non-admin user, even when loading', () => {
      renderHeaderWithAuth({ loading: true, isAuthenticated: true, user: mockUserWhileLoading });
      expect(screen.getByRole('link', { name: 'My Profile' })).toBeInTheDocument();
      expect(screen.getByRole('button', { name: 'Log Out' })).toBeInTheDocument();
      expect(screen.queryByRole('link', { name: 'Admin Inquiries' })).not.toBeInTheDocument();
    });
