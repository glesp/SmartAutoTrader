...
//     preferredVehicleTypes?: string[] | number[]  // Accept either string[] or number[]
//     preferredFuelTypes?: string[] | number[]     // Accept either string[] or number[]
...
//   // If it has $values property (direct ASP.NET format)
...
//   // Fix the type issue by explicitly typing the 'img' parameter
//   const primaryImage = images.find((img: VehicleImage) => img.isPrimary)
//   return (
...
interface Vehicle {
  ...
  vehicleType: number | string
  images?: VehicleImage[] | { $values: VehicleImage[] }
  imageUrl?: string // Add this field for direct access
}
...
const VehicleRecommendations = ({ 
  ...
  console.log("Raw recommendedVehicles:", recommendedVehicles);
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ background: '#f0f0f0', padding: '10px', marginBottom: '10px' }}>
        ...
        <p>Raw prop type: {recommendedVehicles ? typeof recommendedVehicles : 'undefined'}</p>
        <p>Is Array: {Array.isArray(recommendedVehicles) ? 'Yes' : 'No'}</p>
        <p>Has $values: {recommendedVehicles && '$values' in recommendedVehicles ? 'Yes' : 'No'}</p>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '20px' }}>
          {vehicles.map((vehicle) => (
            <div key={vehicle.id} style={{ border: '1px solid #ccc', padding: '10px' }}>
              ...
              <p>Type: {typeof vehicle.vehicleType === 'string' ? vehicle.vehicleType : 'Unknown'}</p>
              <p>Fuel: {typeof vehicle.fuelType === 'string' ? vehicle.fuelType : 'Unknown'}</p>
              {vehicle.imageUrl && <img src={vehicle.imageUrl} alt="Vehicle" style={{ width: '100%' }} />}
            </div>
          ))}
        </div>
      )}
