    ...
    public static class RecommendationParameterValidator
    {
        public static bool Validate(RecommendationParameters parameters, out string errorMessage)
        {
            ...
            if (parameters.PreferredFuelTypes?.Any() == true)
            {
                ...
                {
                    errorMessage = $"Invalid FuelType values: {string.Join(", ", invalidFuelTypes)}. Valid options are: {string.Join(", ", Enum.GetNames(typeof(FuelType)))}";
                    return false;
                }
            ...
            if (parameters.PreferredVehicleTypes?.Any() == true)
            {
                ...
                {
                    errorMessage = $"Invalid VehicleType values: {string.Join(", ", invalidVehicleTypes)}. Valid options are: {string.Join(", ", Enum.GetNames(typeof(VehicleType)))}";
                    return false;
                }
            ...
            {
                errorMessage = $"Invalid price range: Min ({parameters.MinPrice}) cannot be greater than Max ({parameters.MaxPrice})";
                return false;
            }
            ...
            {
                errorMessage = $"Invalid year range: Min ({parameters.MinYear}) cannot be greater than Max ({parameters.MaxYear})";
                return false;
            }
