...
public class ConversationContextService : IConversationContextService
{
    ...
    private readonly Dictionary<int, ConversationContext> _activeContexts = new();
    ...
    public async Task<ConversationContext> GetOrCreateContextAsync(int userId)
    {
        try
        {
            ...
            if (session != null)
            {
                ...
                if (_activeContexts.TryGetValue(userId, out var cachedContext)) return cachedContext;
            ...
            if (session == null) await StartNewSessionAsync(userId);
