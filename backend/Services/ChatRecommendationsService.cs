...
public class ChatRecommendationService : IChatRecommendationService
{
    ...
    public async Task<ChatResponse> ProcessMessageAsync(int userId, ChatMessage message)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(message.ConversationId) &&
                int.TryParse(message.ConversationId, out var sessionId))
            {
            ...
            var conversationContext = await _contextService.GetOrCreateContextAsync(userId);
            ...
            var messageToProcess = message.Content;

            // Check for conversation continuity
            var isFollowUpQuery = IsFollowUpQuery(message.Content, conversationContext);
            ...
            _logger.LogInformation("About to call parameter extraction for message: {MessageContent}",
                messageToProcess);

            var sw = Stopwatch.StartNew();
            var extractedParameters = await ExtractParametersAsync(messageToProcess);
            sw.Stop();
            ...
            if (extractedParameters.IsOffTopic && !string.IsNullOrEmpty(extractedParameters.OffTopicResponse))
            {
                ...
                await SaveChatHistoryAsync(userId, message, extractedParameters.OffTopicResponse,
            ...
            var needsClarification = NeedsClarification(parameters, messageToProcess);
            ...
            var vehicleIds = recommendations.Select(v => v.Id).ToList();
            ...
            var responseMessage = GenerateResponseMessage(parameters, recommendations.Count(), conversationContext);
    ...
    private bool IsFollowUpQuery(string message, ConversationContext context)
    {
        ...
        if (context.MessageCount <= 1 || string.IsNullOrEmpty(context.LastUserIntent)) return false;
        ...
        string[] followUpIndicators = new[]
        {
            "instead", "also", "but", "what about", "how about",
            "can you", "show me", "i want", "i prefer", "i'd like",
            "actually", "change", "modify", "update", "rather"
        };

        if (followUpIndicators.Any(indicator => lowerMessage.Contains(indicator))) return true;

        // Check for pronouns that might refer to previous context
        string[] contextualPronouns = new[] { "it", "that", "these", "those", "them" };
        if (contextualPronouns.Any(pronoun => lowerMessage.Contains($" {pronoun} "))) return true;
    ...
    {
        string[] featureKeywords = new[]
        {
            "bluetooth", "navigation", "leather", "sunroof", "camera",
            "cruise control", "parking sensors", "heated seats", "air conditioning",
            "automatic", "manual", "safety", "fuel efficient", "family", "sport"
        };

        var lowerMessage = message.ToLower();

        foreach (var feature in featureKeywords)
        ...
        if (lowerMessage.Contains("not ") || lowerMessage.Contains("don't want") ||
            ...
            foreach (var make in new[] { "bmw", "audi", "ford", "toyota", "honda" })
        ...
        RecommendationParameters newParams)
    {
        ...
        // Max mileage
        if (mergedParams.MaxMileage == null)
    ...
    private string GenerateClarificationMessage(
        RecommendationParameters parameters,
        string message,
        ConversationContext context)
    {
        var clarification = new StringBuilder();

        // If this is a follow-up question, acknowledge the previous context
        if (context.MessageCount > 1) clarification.Append("Building on our previous conversation, ");

        clarification.Append("I'd like to help you find the perfect vehicle, but I need a bit more information. ");
        ...
        ConversationContext context)
    {
        ...
        if (recommendationCount == 0)
        {
            ...
            if (parameters.MinPrice.HasValue && parameters.MaxPrice.HasValue &&
                ...
                response.Append("Consider broadening your price range. ");

            if (parameters.PreferredMakes?.Count == 1)
                response.Append("Try including more manufacturers in your search. ");

            if (parameters.PreferredVehicleTypes?.Count == 1)
                response.Append("Consider exploring different vehicle types. ");

            return response.ToString();
        }

        // Personalize based on conversation history
        if (context.MessageCount > 1) response.Append("Based on our conversation, ");
    ...
    private async Task<RecommendationParameters> ExtractParametersAsync(string message)
    {
        try
        {
            ...
            var endpoint = _configuration["Services:ParameterExtraction:Endpoint"] ??
                           "http://localhost:5006/extract_parameters";
            var timeoutSeconds = int.TryParse(_configuration["Services:ParameterExtraction:Timeout"], out var timeout)
                ? timeout
                : 30;
            ...
            parameters.MinPrice = jsonDoc.RootElement.TryGetProperty("minPrice", out var minPriceElement) &&
                                  minPriceElement.ValueKind == JsonValueKind.Number
                ? minPriceElement.GetDecimal()
                : null;

            parameters.MaxPrice = jsonDoc.RootElement.TryGetProperty("maxPrice", out var maxPriceElement) &&
                                  maxPriceElement.ValueKind == JsonValueKind.Number
                ? maxPriceElement.GetDecimal()
                : null;

            parameters.MinYear = jsonDoc.RootElement.TryGetProperty("minYear", out var minYearElement) &&
                                 minYearElement.ValueKind == JsonValueKind.Number
                ? minYearElement.GetInt32()
                : null;

            parameters.MaxYear = jsonDoc.RootElement.TryGetProperty("maxYear", out var maxYearElement) &&
            ...
            parameters.PreferredMakes = jsonDoc.RootElement.TryGetProperty("preferredMakes", out var makesElement) &&
                                        makesElement.ValueKind == JsonValueKind.Array
                ? makesElement.EnumerateArray().Where(e => e.ValueKind == JsonValueKind.String)
                    .Select(e => e.GetString()).ToList()
                : new List<string>();

            parameters.DesiredFeatures =
                jsonDoc.RootElement.TryGetProperty("desiredFeatures", out var featuresElement) &&
            ...
            if (jsonDoc.RootElement.TryGetProperty("preferredFuelTypes", out var fuelTypesElement) &&
            ...
            if (jsonDoc.RootElement.TryGetProperty("preferredVehicleTypes", out var vehicleTypesElement) &&
                vehicleTypesElement.ValueKind == JsonValueKind.Array)
                parameters.PreferredVehicleTypes = vehicleTypesElement.EnumerateArray()
                    .Where(e => e.ValueKind == JsonValueKind.String)
                    .Select(e =>
                        Enum.TryParse<VehicleType>(e.GetString(), true, out var vehicle) ? vehicle : (VehicleType?)null)
                    .Where(v => v.HasValue)
                    .Select(v => v.Value)
                    .ToList();
            else
